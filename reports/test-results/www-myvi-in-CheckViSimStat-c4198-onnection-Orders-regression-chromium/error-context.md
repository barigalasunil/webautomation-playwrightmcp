...
- Name: www-myvi-in\CheckViSimStatusOnlineTrackYourConnectionOrders.spec.ts >> https://www.myvi.in/ - Check Vi SIM Status Online | Track Your Connection & Orders - Regression Tests >> Verify that 25 Links are present on Check Vi SIM Status Online | Track Your Connection & Orders @regression
- Location: tests\generated\www-myvi-in\CheckViSimStatusOnlineTrackYourConnectionOrders.spec.ts:96:3
...
Locator: getByRole('link', { name: 'go back to order tracking' }).first()
Expected: attached
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('link', { name: 'go back to order tracking' }).first()
