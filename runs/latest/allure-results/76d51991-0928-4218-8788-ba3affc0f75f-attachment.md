...
- Name: www-myvi-in\PortSimNumberToViEnjoyViBenefitsWithEasyMnpProcess.spec.ts >> https://www.myvi.in/ - Port SIM Number to Vi | Enjoy Vi Benefits with Easy MNP process - Regression Tests >> Verify that 2 Menus are present on Port SIM Number to Vi | Enjoy Vi Benefits with Easy MNP process @regression
- Location: tests\generated\www-myvi-in\PortSimNumberToViEnjoyViBenefitsWithEasyMnpProcess.spec.ts:264:3
...
Locator: getByRole('navigation', { name: 'home new connection mobile number portability (MNP)' }).first()
Expected: attached
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('navigation', { name: 'home new connection mobile number portability (MNP)' }).first()
