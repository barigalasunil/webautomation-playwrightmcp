...
function isTextContentPresent(value: string): boolean {
...
export async function analyzePage(page: Page): Promise<SiteMapPage> {
  ...
  // Wait for the page to settle so that dynamically loaded content is included.
  try {
  ...
  const forms = await page.$$eval("form", (elements) =>
    elements.map((element) => ({
      ...
      fieldCount: element.querySelectorAll("input, textarea, select, button").length,
    }))
  );
  ...
  const dropdowns = await page.$$eval("select, [role=combobox]", (elements) =>
    elements.map((element) => {
      ...
      return {
        ...
        optionCount: element.tagName.toLowerCase() === "select" ? element.querySelectorAll("option").length : 0,
      };
    })
  );
